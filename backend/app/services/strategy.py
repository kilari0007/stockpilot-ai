...
def evaluate(ticker: str, stock: pd.DataFrame, spy: pd.DataFrame) -> dict:
    ...
    label="STRONG BUY" if passed==len(checks) else "WATCH" if passed>=5 else "AVOID"
    risk=2*x.atr; entry_low=x.close*.998; entry_high=x.close*1.002
    return {"ticker":ticker.upper(),"label":label,"confidence":confidence,"price":float(round(x.close,2)),"entry_low":float(round(entry_low,2)),"entry_high":float(round(entry_high,2)),"stop":float(round(x.close-risk,2)),"target":float(round(x.close+.75*risk,2)),"rationale":", ".join(k for k,v in checks.items() if v)}
